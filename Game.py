import hiragana_sets, katakana_sets
import pygame
import pygame_menu
...
class OppaiType:
    def __init__(self):
        ...
        self._hiragana_sets = hiragana_sets.vowels + hiragana_sets.k + hiragana_sets.s + hiragana_sets.t
    ...
    def setup_menus(self):
        # Main
        self._main_menu = pygame_menu.Menu('Main Menu', self.weight, self.height,
            theme=self.menu_theme)
        self._main_menu.add.button('Play', self.start_game)
        self._main_menu.add.button('Options', self.options_menu)
        self._main_menu.add.button('Quit', pygame_menu.events.EXIT)
        # Options
        self._options_menu = pygame_menu.Menu('Options', self.weight, self.height,
            theme=self.menu_theme)
        self._options_menu.add.dropselect_multiple('Hiragana Sets', [('あ ...', hiragana_sets.vowels),
                                                                     ('か ...', hiragana_sets.k),
                                                                     ('が ...', hiragana_sets.g),
                                                                     ('さ ...', hiragana_sets.s),
                                                                     ('ざ ...', hiragana_sets.z),
                                                                     ...
                                                                     ('わ ...', hiragana_sets.w),],
                                                                     default=[0, 1, 3, 5],
                                                                     onreturn=lambda value: setattr(self, '_hiragana_sets', ([item for _, h_set in value[0] for item in h_set])))
        self._options_menu.add.dropselect_multiple('Katakana Sets', [('ア ...', katakana_sets.vowels),
                                                                     ('カ ...', katakana_sets.k),
                                                                     ('ガ ...', katakana_sets.g),
                                                                     ('サ ...', katakana_sets.s),
                                                                     ('ザ ...', katakana_sets.z),
                                                                     ...
                                                                     ('ワ ...', katakana_sets.w),],
                                                                     default=[0, 1],
                                                                     onreturn=lambda value: setattr(self, '_katakana_sets', ([item for _, h_set in value[0] for item in h_set])))
        self._options_menu.add.range_slider('Time Limit', default=45, range_values=(10, 120), increment=1,
    ...
    def options_menu(self):
        self._main_menu._open(self._options_menu)
    ...
    def on_render(self):
        ...
        self._display_surf.blit(self.font.render(f'High Score: {self._high_score}', True, (100, 200, 200)), (50, 10))
        if self._round_running:
            self._display_surf.blit(self.font.render(f'Time Remaining: {self._time_remaining} s', True, (100, 200, 200)), (50, 50))
            _, prompt = self._prompt_queue[0]
            self._display_surf.blit(self.font.render(f'{prompt}', True, (100, 200, 200)), dest=(50, 100))
            self._display_surf.blit(self.font.render(f'Input: {self._user_input}', True, (100, 200, 200)), dest=(50, 150))
            self._display_surf.blit(self.font.render(f'Score: {self._score}', True, (100, 200, 200)), dest=(50, 200))
        else:
            self._display_surf.blit(self.font.render(f'Press space to start', True, (100, 200, 200)), (50, 50))
        if self.missed_characters:
                self._display_surf.blit(self.font.render(f'Missed: {", ".join(f"{c}:{n}" for n, c in self.missed_characters)}', True, (200, 100, 100)), dest=(50, 250))
        pygame.display.flip()
